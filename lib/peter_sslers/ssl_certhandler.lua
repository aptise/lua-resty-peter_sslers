...
function api_response_to_cert(server_name, response, cert_preferences)
    ...
    if not response then
        ...
        if status == 200 then
            ...
            if body_value ~= nil then
                ...
                if cert_preferences == nil then
                    cert_preferences = {'server_certificate__latest_multi', 'server_certificate__latest_single'}
                end
        ...
        elseif status == 404 then
            ...
            if body_value ~= nil then
                ...
                ngx_log(ngx_DEBUG, "API query error : ", server_name, " : ", body_value['message'])
            end
        else
            ngx_log(ngx_ERR, 'API query - bad response : ', server_name , " : ", status)
        end
    end
...
function query_api_upstream(fallback_server, server_name, cert_preferences)
...
function query_api_autocert(fallback_server, server_name)
    -- fallback_server : http(s) server root for peter_sslers installation
    -- server_name : active `ssl.server_name`
    -- cert_preferences : array of api keys to prefer
    --
...
function set_ssl_certificate(redis_strategy, fallback_server, enable_autocert, cert_preferences)
    ...
    --                    should be {"server_certificate__latest_single", "server_certificate__latest_multi"}
    --
